...
typedef struct {
     ...
     * The pointer must be valid until the call to console_deinit.
     */
    const char* command;    //!< command name
    /**
     * Help text for the command, shown by help command.
     * If set, the pointer must be valid until the call to console_deinit.
     ...
     * Only used for the duration of console_cmd_register call.
     */
    void* argtable;
} console_cmd_t;
 ...
 * @return
 *      - OK, if command was run
 *      - ERR_INVALID_ARG, if the command line is empty, or only contained
 *        whitespace
 *      - ERR_NOT_FOUND, if command with given name wasn't registered
 *      - ERR_INVALID_STATE, if console_init wasn't called
 */
