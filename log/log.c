...
typedef struct uncached_tag_entry_{
    ...
    uint8_t level;  // log_level_t as uint8_t
...
static inline void add_to_cache(const char* tag, log_level_t level);
static void heap_bubble_down(int index);
static inline void heap_swap(int i, int j);
static inline bool should_output(log_level_t level_for_message, log_level_t level_for_tag);
...
void IRAM_ATTR log_write(log_level_t level,
        const char* tag,
        const char* format, ...)
{
    if (!s_log_mutex) {
...
static inline bool get_cached_log_level(const char* tag, log_level_t* level)
{
    ...
    *level = (log_level_t) s_log_cache[i].level;
...
uint32_t ATTR log_early_timestamp()
{
	return 0;
...
uint32_t IRAM_ATTR log_timestamp()
{
...
uint32_t log_timestamp() __attribute__((alias("log_early_timestamp")));
